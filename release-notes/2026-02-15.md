...
- **功能更新**
  ...
  - ⭐ Cron/Gateway: 支援完成run webhook遞送開關與 independent webhook auth
  - ⭐ Channels: 簡化核心與擴展的探針/token解析基類

- **安全性提升**
  - ⭐⭐⭐ Sandbox: 將SHA-1雜湊取代為SHA-256
  - ⭐⭐ Security/Logging: 從錯誤訊息中脫敏Telegram bot token
  - ⭐⭐⭐ Sandbox/Security: 阻止危險的Docker設定
  - ⭐ Sandbox: 保留陣列順序於配置雜湊
  - ⭐⭐ Gateway/Security: 從非管理用戶的狀態回應中脫敏敏感資訊
  - ⭐ Gateway/Control UI: 維護Control UI bypass mode的運算元範圍
  - ⭐ LINE/Security: 缺少channel token或secret時關閉webhook
  - ⭐⭐ Skills/Security: 限制下載安裝程序的targetDir
  - ⭐ Skills/Linux: 增強apt系統上的go安裝程序
  - ⭐ Web Fetch/Security: 限制下載回應主體大小
  - ⭐ Config/Gateway: 使敏感鍵白名稱匹配不區分大小寫
  - ⭐ Dev tooling: 增強git pre-commit hook防範惡意檔案名稱
  - ⭐ Gateway/Agent: 拒絕格式錯誤的agent:-prefix session keys
  - ⭐⭐ Gateway/Chat: 增強聊天.send傳入訊息處理
  - ⭐ Gateway/Send: 為webchat目標返回可操作錯誤
  - ⭐⭐⭐ Control UI: 防止助手名稱/頭像的XSS
  - ⭐⭐ Agents/Security: 清理工作區路徑
  - ⭐ Agents/Sandbox: 澄清系統提示路徑指導
  - ⭐ Agents/Context: 在提供商發現後應用contextWindow覆蓋
  - ⭐ Agents/OpenAI: 為直接OpenAI Responses/Codex強制store=true
  - ⭐ Memory/FTS: Unicode感知的查詢令牌
  - ⭐ Agents: 為嵌入式run超時返回明確超時錯誤
  - ⭐⭐ Group chats: 每個turn注入群組聊天上下文
  - ⭐ Browser/Agents: 當瀏覽器控制服務不可用時返回明確非重試指導

- **錯誤修復**
  - ⭐⭐⭐ Telegram: 為DM發送省略message_thread_id
  - ⭐⭐⭐ Telegram: 重試傳入媒體getFile呼叫
  - ⭐⭐⭐ Telegram: 原地完成streaming preview回覆
  - ⭐⭐ Telegram: 當channels.telegram.streamMode為off時禁用block streaming
  - ⭐⭐ Discord: 當runtime payload省略message.channelId時保留channel session連續性
  - ⭐ Discord: 在多代理設置中按技能名稱去重原生技能命令
  - ⭐ Discord: 確保role allowlist匹配使用原始role ID
  - ⭐ Web UI/Agents: 在啟動完成後隱藏Agents Files list中的BOOTSTRAP.md
  - ⭐⭐ Memory/QMD: 為每個代理範圍管理的集合名稱
  - ⭐ Auto-reply/WhatsApp/TUI/Web: 當最終助手訊息為NO_REPLY且訊息工具送信成功時，鏡像送信的訊息文字

---

## 功能更新 (Features)

### 1. Discord Components v2 支援 ⭐⭐⭐
...
### 6. Discord: 根據技能名稱去重原生技能命令 ⭐
- **用途**：在多代理設置中根據技能名稱去重原生技能命令
- **解決問題**：出現帶有 _2 後綴的重複斜線命令
- **影響**：避免命令重複 (#17365)

### 7. Discord: Role Allowlist 匹配原始 Role ID ⭐
- **用途**：確保 role allowlist 匹配使用原始 role ID 進行訊息路由授權
- **解決問題**：role ID 匹配不正確
- **影響**：確保正確的授權機制

### 8. Web UI/Agents: 隱藏完成启動後的 BOOTSTRAP.md ⭐
...
| 類別 | 數量 | 重點摘要 |
|------|------|----------|
| 功能更新 | 6+ | Discord Components v2、Plugins Hooks、巢狀 Subagents |
| 安全性 | 20+ | Sandbox 增強、密鑰脫敏、Path 清理、XSS 防護 |
| 錯誤修復 | 12+ | Telegram DM、Discord Session、Memory QMD |
